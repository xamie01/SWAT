...
export async function upsertWallet(wallet) {
    const rows = await query(`INSERT INTO wallets (address, source, nickname, discovery_method)
     VALUES ($1, $2, $3, $4)
     ON CONFLICT (address) DO UPDATE SET
       ...
       updated_at = NOW()
     RETURNING *`, [wallet.address, wallet.source, wallet.nickname ?? null, wallet.discoveryMethod ?? null]);
    return rows[0];
}
 ...
 */
export async function deleteWalletCascade(address) {
