...
import { getWalletMetrics, updateWalletScore, pauseUnderperformingWallets, promoteHighScoringDiscoveredWallets, reactivateDormantWallets, pruneUnprofitableBigBuyers, getActiveWallets, generateFundingClusters, generateBehavioralClusters, markToMarketOpenPositions } from '@swat/db';
...
async function runScoringBatch() {
    ...
    if (pruned > 0)
        console.log(`[scorer] Pruned ${pruned} unprofitable token-big wallets`);
...
function subscribeClusterRefresh() {
    subscriber.subscribe(REDIS_CHANNELS.clusterRefresh, (err) => {
        if (err)
            console.error('[scorer] subscribe failed', err);
        else
            console.log('[scorer] subscribed to cluster-refresh channel.');
    });
}
...
subscriber.on('message', (channel) => {
    if (channel !== REDIS_CHANNELS.clusterRefresh)
        return;
    if (clusterRefreshing)
        return;
