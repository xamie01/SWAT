...
import { insertParsedTransaction, refreshWalletActivity, upsertToken, upsertWallet, getTokenDecimals } from '@swat/db';
...
function getWalletTokenDeltas(tx, walletAddress) {
    const pre = tx.meta?.preTokenBalances ?? [];
    const post = tx.meta?.postTokenBalances ?? [];
    const balances = new Map();
    for (const item of pre) {
        if (item.owner !== walletAddress || !item.mint)
            continue;
    ...
    if (!tokenIn || !tokenOut)
        return null;
    const direction = tokenIn.mint === SOL_MINT ? 'buy' : tokenOut.mint === SOL_MINT ? 'sell' : 'buy';
...
async function enrichTransactionUsd(tokenIn, tokenOut, amountInStr, amountOutStr, targetToken) {
    ...
    if (targetPrice) {
        ...
        if (decimals === null)
            decimals = 6; // safe default for most SPL tokens
...
async function backfillWallet(address) {
    ...
    const signatures = await heliusRpc('getSignaturesForAddress', [address, { limit: 1000 }]);
    if (signatures.length === 0) {
        await refreshWalletActivity(address, null, 0);
        return { fetched: 0, inserted: 0, skipped: 0 };
    }
    const signatureList = signatures.map((s) => s.signature);
    const txs = await heliusRpc('getParsedTransactions', [
        signatureList,
