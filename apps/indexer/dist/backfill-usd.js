...
async function run() {
    ...
    const txs = await query(`SELECT id, token_in, token_out, amount_in, amount_out 
     FROM transactions 
    ...
    for (const tx of txs) {
        ...
        if (amountInUsd !== null && amountOutUsd !== null) {
            await query(`UPDATE transactions SET amount_in_usd = $1, amount_out_usd = $2 WHERE id = $3`, [amountInUsd, amountOutUsd, tx.id]);
            updated++;
        }
...
//# sourceMappingURL=backfill-usd.js.map