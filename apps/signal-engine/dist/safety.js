...
const heliusRpcUrl = heliusApiKey
    ...
    : null;
async function getMintInfoFromChain(tokenMint) {
    if (!heliusRpcUrl)
        return null;
    try {
...
async function getTop10HolderPct(tokenMint) {
    if (!heliusRpcUrl)
        return null;
    try {
        const response = await fetch(heliusRpcUrl, {
...
async function getDexScreenerLiquidity(tokenMint) {
    try {
...
export async function checkTokenSafety(tokenMint) {
    ...
    if (!mintInfo) {
        ...
        const rows = await query(`SELECT mint_authority_disabled, freeze_authority_disabled FROM tokens WHERE mint = $1`, [tokenMint]);
        const token = rows[0];
        if (!token) {
            return {
                isSafe: false,
